...
func (s *authSvc) Get(ctx context.Context, ID string) (playlist PlaylistResp, err error) {
...
func (s *authSvc) Add(ctx context.Context, title string, description string, songIDs []string) (playlistID string, err error) {
	_, err = s.verifyToken(ctx)
	if err != nil {
		return
	}
...
func (s *authSvc) List(ctx context.Context) (playlists []PlaylistResp, err error) {
	_, err = s.verifyToken(ctx)
	if err != nil {
		return
	}
